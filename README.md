# Data Structures Roadmap

# Table of Contents

1. [Big O Notation](Big%20O%20Notation.md)
    - [General Explanation](Big%20O%20Notation.md#general-explanation)
    - [Rulues of Big O](Big%20O%20Notation.md#rulues-of-big-o)
    - [Common Big O Notations](Big%20O%20Notation.md#common-big-o-notations)
    - [Interview Questions](Big%20O%20Notation.md#interview-questions)
2. [Arrays](Arrays.md)
    - [General Information](Arrays.md#general-information)
    - [Common Big O Notations](Arrays.md#common-big-0-notataions)
        - [1. Lookup by index](Arrays.md#1-lookup-by-index)
        - [2. Lookup by value](Arrays.md#2-lookup-by-value)
        - [3. Array traversal](Arrays.md#3-array-traversal)
        - [4. Array insertion](Arrays.md#4-array-insertion)
        - [5. Array deletion](Arrays.md#5-array-deletion)
    - [Interview Questions](#interview-questions)

